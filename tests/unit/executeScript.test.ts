...
import { buildNextAction, getPlanSummary, markCompleted, markFailed, resetTask, runCli, stamp } from '../../tools/scripts/execute.mjs';
...
describe('buildNextAction', () => {
  it('Worker for pending', () => {
    const s = getPlanSummary(mk());
    expect(buildNextAction(s)).toEqual(expect.objectContaining({
      kind: 'delegate', agent: 'Worker', taskName: 'task-2-core'
    }));
  });
  it('error for failed', () => {
    const d = mk();
    ...
    expect(buildNextAction(getPlanSummary(d))).toEqual({
      kind: 'error', message: 'Task task-2-core 실패.\nfail'
    });
  });
});
...
describe('runCli', () => {
  const so = () => { let o = ''; return { stdout: { write: (v: string) => { o += v; } }, stderr: { write() {} }, out: () => o }; };
  it('next', () => {
    const d = mk(), m = so();
  ...
  it('complete', () => {
    const d = mk(), m = so();
    expect(runCli(['complete', 'plan-1', 'task-2-core', '--summary', 'ok'], { rootDir: d, ...m })).toBe(0);
